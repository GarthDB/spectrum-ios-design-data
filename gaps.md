...
- `lightIncreased`/`darkIncreased` naming maps to `contrast:"high"` — modeled here, and now
  registered via `extensions/platform-extensions/ios-contrast.json` (auto-discovered platform
  extension), so the increased→high crosswalk is discoverable, not just implicit in the importer.
